...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{df} \hlkwb{<-} \hlkwd{lmerTestContrast}\hlstd{(fm, lT)}
\hlstd{df} \hlkwb{<-} \hlstd{df} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{Contrast} \hlstd{=} \hlstr{'Mutant-Control'}\hlstd{)}
\hlstd{df}\hlopt{$}\hlstd{SE} \hlkwb{<-} \hlkwd{round}\hlstd{(df}\hlopt{$}\hlstd{SE,}\hlnum{3}\hlstd{)}
\hlstd{df}\hlopt{$}\hlstd{log2FC} \hlkwb{<-} \hlkwd{round}\hlstd{(df}\hlopt{$}\hlstd{log2FC,}\hlnum{3}\hlstd{)}
\hlstd{df}\hlopt{$}\hlstd{percentControl} \hlkwb{<-} \hlkwd{round}\hlstd{(df}\hlopt{$}\hlstd{percentControl,}\hlnum{3}\hlstd{)}
\hlstd{df}\hlopt{$}\hlstd{Tstatistic} \hlkwb{<-} \hlkwd{round}\hlstd{(df}\hlopt{$}\hlstd{Tstatistic,}\hlnum{3}\hlstd{)}
\hlstd{df}\hlopt{$}\hlstd{Pvalue} \hlkwb{<-} \hlkwd{formatC}\hlstd{(df}\hlopt{$}\hlstd{Pvalue,}\hlkwc{digits}\hlstd{=}\hlnum{3}\hlstd{)}
\hlstd{df}\hlopt{$}\hlstd{isSingular} \hlkwb{<-} \hlkwa{NULL}
\hlstd{df} \hlopt{%>%} \hlkwd{unique}\hlstd{()} \hlopt{%>%} \hlstd{knitr}\hlopt{::}\hlkwd{kable}\hlstd{()}
\end{alltt}
\end{kframe}
\begin{tabular}{l|r|r|r|r|l|r}
\hline
...
measure of the quality of a mixed model is Nagagawa's coefficient of 
determination. Nakagawa's conditional $R^2$ is interpreted as 
...
\hlkwd{r.squaredGLMM.merMod}\hlstd{(fm)} \hlopt{%>%} \hlstd{knitr}\hlopt{::}\hlkwd{kable}\hlstd{()}
\end{alltt}
\end{kframe}
\begin{tabular}{r|r}
\hline
