...
\hlstd{fit} \hlkwb{<-} \hlkwd{lmer}\hlstd{(fx, msstats_prot} \hlopt{%>%} \hlkwd{filter}\hlstd{(Protein} \hlopt{%in%} \hlstd{washc_prots))}
...
\hlstd{mixef_var} \hlkwb{<-} \hlkwd{setNames}\hlstd{(var_df}\hlopt{$}\hlstd{vcov,}\hlkwc{nm}\hlstd{=var_df}\hlopt{$}\hlstd{grp)} \hlcom{# var of each component}
\hlstd{mixef_var}\hlopt{/}\hlkwd{sum}\hlstd{(mixef_var)} \hlcom{# as a percent of the total mixed-effect variance}
\end{alltt}
\begin{verbatim}
...
\hlstd{fit} \hlkwb{<-} \hlkwd{lmer}\hlstd{(fx,} \hlkwc{data} \hlstd{= msstats_prot} \hlopt{%>%} \hlkwd{filter}\hlstd{(Protein} \hlopt{%in%} \hlstd{washc_prots))}
