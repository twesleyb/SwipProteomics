...
Previously, we our analysis relied upon the use of the R package \texttt{edgeR}
to perform statistical inference at the level of proteins and protein groups or
modules. In the review of this manuscript, concerns were raised about the
appropriatness of \texttt{edgeR} for TMT protein mass spectrometry data.\\
...
However, we failed to thoughougly consider the overall adequacy of the NB
framework for mass spectrometry data. Here we reconsider its appropriatness 
...
parameterized by a dispersion parameter, $\phi$. \\
