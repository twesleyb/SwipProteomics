...
Using \texttt{MSstatsTMT} we assesed two different types of contrasts:\\

\begin{enumerate}
	\item 'intra-BioFraction' comparisons 
	\item 'Mutant-Control' contrast 
\end{enumerate}

For each protein, we assssed the difference between Mutant and Control for all
seven of the \texttt{BioFractions} and for the overall difference between 
'Mutant' and 'Control' conditions

Each contrast is represented bya vector, $l^T$, which specifies a
comparison between coefficients in the LMM. Figure (FIG) shows these two
