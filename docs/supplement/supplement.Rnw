...
defaults <- list(tidy=FALSE, message=FALSE, warning=FALSE, 
	     fig.width=4, fig.height=4.5, results='hide')
...
% NOTE: knitr chunk options are inherited from the global options set above.
% I've set results='hide' as the default. So you must explicitly set
% results to 'markup' for results that you want to show.

<<fit0>>=
@

<<tab0, echo=FALSE, results="markup">>=
@

<<contrast7, echo = FALSE>>=
@

<<contrast8, echo = FALSE>>=
@
