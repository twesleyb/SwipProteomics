...
\begin{abstract}
	...
	experiment, network construction and community detection, and approach to perform
	protein- and module-level statistical testing. evaluate
	protein level differential abundance using \texttt{MSstatsTMT}. We
	extend Huang et al.'s linear mixed-model (LMM) framework to perform 
