...
dataProcessPlotsTMT <- function(data.peptide,
  ...
  if (toupper(type) == "PROFILEPLOT") {
    ...
    if (which.Protein != "all") {
      ...
      if (is.character(which.Protein)) {
        ...
        if (length(setdiff(temp.name, unique(datafeature$Protein))) > 0) {
          stop(paste0(
            "Please check protein name. Data set does not have this protein. - ",
            toString(temp.name)
          ))
        }
      }
      ...
      if (is.numeric(which.Protein)) {
        ...
        if (length(levels(datafeature$Protein)) < max(which.Protein)) {
          stop(paste0(
            "Please check your ion of proteins. There are ",
    ...
    groupline <- tempGroupName %>%
      dplyr::group_by(Condition, Run) %>%
      dplyr::mutate(groupAxis = n())
    ...
    groupline <- groupline %>%
      dplyr::group_by(Run) %>%
      dplyr::mutate(cumGroupAxis = cumsum(groupAxis))
    ...
    if (originalPlot) {
      ...
      for (i in seq_len(nlevels(datafeature$Protein))) {
        ...
        if (nrow(sub) == sum(is.na(sub$abundance))) {
          message(paste0(
            "Can't the Profile plot for ", unique(sub$Protein),
        ...
        ptemp <- ggplot(aes_string(
          ...
          guides(
            color = guide_legend(
              title = paste("# peptide:", nlevels(sub$PeptideSequence)),
        ...
        message(paste(
          "Drew the Profile plot for ", unique(sub$Protein),
          "(", i, " of ", length(unique(datafeature$Protein)), ")"
        ))
      }
      ...
      for (i in seq_len(nlevels(datafeature$Protein))) {
        ...
        if (haverun) {
          ...
          ptempall <- ggplot(aes_string(
            x = "xorder", y = "abundance",
            color = "analysis", linetype = "PSM", size = "analysis"
          ), data = final) +
            facet_grid(~Run) +
            geom_point(size = dot.size.profile) +
            geom_line(size = 0.5) +
          ...
          message(paste(
            "Drew the Profile plot with summarization for ", unique(sub$Protein),
            "(", i, " of ", length(unique(datafeature$Protein)), ")"
          ))
        }
      } # end-loop for each protein
  ...
  if (toupper(type) == "QCPLOT") {
    ...
    if (which.Protein != "allonly") {
      ...
      for (i in seq_len(nlevels(datafeature$Protein))) {
        ...
        if (nrow(sub) == sum(is.na(sub$abundance))) {
          message(paste(
            "Can't the Quality Control plot for ", unique(sub$Protein),
            "(", i, " of ", length(unique(datafeature$Protein)),
            ") because all measurements are NAs."
          ))
          next()
        ...
        message(paste(
          "Drew the Quality Contol plot(boxplot) for ", unique(sub$Protein),
          "(", i, " of ", length(unique(datafeature$Protein)), ")"
        ))
      } # end-loop
    }
