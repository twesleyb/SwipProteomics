...
testContrasts <- function(fit_list, contrast_matrix, 
			  moderated=FALSE, padj_method="BH") {
...
for (protein in proteins) {
  ...
  # We compute the unmoderated t-statistic:
  df.prior <- 0
  s2.prior <- 0
  # for the given protein loop to perform tests for every comparison
  # FIXME: collapse loop as FUNCTION!
  ...
  rho$stats <- df
  # update fit_list
  fit_list[[protein]] <- rho
} # #EOL for proteins
# return rho updated with protein stats
# NOW PERFORM MODERATION:
# TO RHO add isModerated  
#    if (moderated) {
