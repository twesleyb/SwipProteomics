#' filtProt
#'
#' filter protein-level data
#'
#' @export filtProt


logRatios <- function(x) {
  # helper function that calculate the log ratio of all comparisons of two
...
filtProt <- function(tp, controls = "SPQC", 
		     rowmax = 0.5, nbins = 5, nSD = 4,
  ...
  tp <- tp_in %>%
    filter(Accession %notin% all_out) %>%
    as.data.table()
  ...
  if (summary) {
    message(paste(
      "Removing proteins identified by one peptide.....:",
      formatC(n_ohw, big.mark = ",")
    ))
    message(paste(
      "Removing proteins with too many missing values..:",
      formatC(n_filt_missing, big.mark = ",")
    ))
    message(paste(
      "Removing proteins with any missing QC values....:",
      formatC(n_filt_qc, big.mark = ",")
    ))
    message(paste(
      "Removing proteins with outlier measurements.....:",
      formatC(n_outliers, big.mark = ",")
    ))
    message(paste(
      "Final number of reproducibly quantified proteins:",
