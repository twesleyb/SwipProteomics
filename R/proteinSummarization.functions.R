...
.protein.summarization.function <- function(data,
  ...
  if (nrow(data[!is.na(data$Intensity) & data$Intensity < 1]) > 0) {
    ...
    warning("** Negative log2 intensities were replaced with NA.")
  }

  ## Record the group information
  annotation <- unique(data[!is.na(data$log2Intensity), c(
  ...
  if (method == "msstats") {
    ...
    for (i in seq_len(num.run)) {
      ...
      res.sub <- res.sub[which(colnames(res.sub) %in% c(
        "Protein",
    ...
    data <- data[!is.na(data$log2Intensity), c(
      "ProteinName", "PSM", "log2Intensity",
    ...
    anno <- anno %>%
      unite("Run.Protein", Run, ProteinName, sep = " ") %>%
    ...
    res <- data[, .(MedianPolish = .medianPolish(
      log2Intensity,
      channel.len
    )), by = .(Run, ProteinName)]
    ...
    res <- data[, .(LogSum = log2(sum(2^log2Intensity))), by = .(
      Run, ProteinName,
      runchannel
    )] # calculate the logsum for each protein and channel
    ...
    res <- data[, .(Median = median(log2Intensity)),
      by = .(Run, ProteinName, runchannel)
    ] # calculate the median for each protein and channel
    colnames(res) <- c("Run", "Protein", "runchannel", "Abundance")
    res <- left_join(res, annotation, by = "runchannel")
  ...
  res <- res[, c(
    "Run", "Protein", "Abundance", "Channel", "BioReplicate",
...
## data: protein level data, which has columns Protein, Group, BioReplicate, Run, Channel, Abundance
#' @keywords internal
.protein.normalization <- function(data) {
  ...
  if (is.element("Norm", group.info)) {
      ...
      if (length(unique(sub_data$Run)) > 1) { # multiple runs
        ...
        if (nrow(norm.channel) > 0) { # normalization channels are existing
          norm.channel <- norm.channel[, .(Abundance = mean(Abundance, na.rm = TRUE)),
            by = .(Protein, Run)
          ] # calculate the mean over multiple normalization channels
    ...
    warning("** 'Norm' information in Condition is required for normalization.
