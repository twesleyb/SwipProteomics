...
for (i in c(1:length(corum_prots))) {
  ...
  if (all(table(part) == 1)) {
    ...
    prots <- names(part[part == as.numeric(names(which(table(part)==max(table(part)))))])
    prot_list[[i]] <- prots
  }
}
...
for (path in names(filt_list)){
  ...
  lmer_args[["data"]] <- msstats_prot %>% subset(Protein %in% prots) %>% group_by(Protein) %>% mutate(scale_Abundance = Abundance/max(Abundance))
