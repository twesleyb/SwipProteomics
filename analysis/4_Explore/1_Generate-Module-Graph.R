...
# Each plots data contains the fitted values.

# Get plots for significant modules.
#plots <- grouped_plots[sig_modules]
plots <- grouped_plots

# Collect data for WT.
data_list <- lapply(plots,function(x) x$data %>% filter(Genotype == "WT"))

# Get Fraction Adjusted.Intensity
dt <- bind_rows(data_list,.id="Module") %>% 
	group_by(Module,Fraction) %>% 
	summarize(Fitted.Intensity=Fitted.Intensity[1],.groups="drop")

# Cast to matrix.
dm <- dt %>% as.data.table() %>% 
	dcast(Module ~ Fraction, value.var = "Fitted.Intensity") %>%
	as.matrix(rownames="Module")
adjm <- WGCNA::bicor(t(dm))
ne_adjm <- neten::neten(adjm)

sub_adjm <- ne_adjm[sig_modules,sig_modules] %>% as.data.table(keep.rownames="Module")

# Save for clustering.
myfile <- file.path(root,"rdata","sig_mod_adjm.csv")
fwrite(sub_adjm,myfile)
