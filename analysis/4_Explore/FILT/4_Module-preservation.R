...
adjm = c(discovery="filt_adjm.csv", test=NULL)
netw = c(discovery="filt_ne_adjm.csv", test=NULL)
part = c(discovery="FiltNE_partition.csv", test=NULL)
...
if (save_as == "rdata") {
	...
	# Save as rda.
	myfile <- file.path(datadir,"filt_partition.rda")
