...
load(file.path(root,"rdata","msstats_partition.rda")) # partition
...
filt_prot <- msstats_prot %>% filter(!is.na(Module)) %>% filter(Module %notin% to_drop)

# the lmer formula:
fx <- formula(Abundance ~ 0 + (1|BioFraction) + (1|Protein) + Genotype)
...
for (module in modules){
  results_list[[module]] <- lmerTestModule(msstats_prot,module,contrast_matrix=cm1)
...
module_size <- sapply(split(partition,partition),length)[as.character(results_df$module)]
