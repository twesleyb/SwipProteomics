...
rootdir <- getrd()
renv::load(rootdir,quiet=TRUE)  # Not sure what this Error is.
...
if (!dir.exists(datadir)) { dir.create(datadir, showWarnings = TRUE) }
if (!dir.exists(downdir)) { dir.create(downdir, showWarnings = TRUE) }
if (!dir.exists(rdatdir)) { dir.create(rdatdir, showWarnings = TRUE) }
if (!dir.exists(tabsdir)) { dir.create(tabsdir, showWarnings = TRUE) }
...
#---------------------------------------------------------------------
## Experiment: analyze data from 1 mouse, or a single fraction.
#---------------------------------------------------------------------

# Mode 1: Analyze data from a single mouse, all fractions.
# Mode 2: Analyze data from multiple mice, single efraction. 

# Keep SPQC? Are SPQC included in the experiment?

n <- 1
animals <- unique(tidy_peptide$"Mouse ID")
idx <- which(is.na(animals))
animals[idx] <- "SPQC"
rand_animal <- sample(animals[animals != "SPQC"],n)
subdat <- tidy_peptide %>% filter(`Mouse ID` %in% c("SPQC",rand_animal))
n_animals <- unique(subdat$"Mouse ID")
n_animals
