...
message(
  "\nTime to perform 9 pairwise comparisons for ", length(proteins),
...
msstats_results$Protein <- as.character(msstats_results$Protein)
idx <- msstats_results$adj.pvalue<FDR_alpha
sigprots <- msstats_results[idx,] %>% select(Protein) %>% unlist() %>% unique()
