...
OpenMStoMSstatsTMTFormat <- function(input,
  ...
  tmp <- input %>%
    dplyr::group_by(ProteinName, PeptideSequence, Charge, Reference, RetentionTime, Run) %>%
  ...
  if (useUniquePeptide) {
    ...
    structure <- pepcount %>%
      ...
      summarise(npro = n_distinct(ProteinName))
...
.check.input <- function(data) {
  required.columns <- c(
    "ProteinName", "PeptideSequence", "Charge", "Run", "TechRepMixture", "Fraction",
  ...
  if (!all(required.columns %in% colnames(data))) {
    ...
    stop(paste(
      "Please check the required column in the input file. ** columns :",
...
.AggregatePSMstoPeptideIons <- function(data, summaryforMultiplePSMs = sum) {
  ...
  input <- data %>%
    dplyr::select(
      RetentionTime, Reference, ProteinName,
