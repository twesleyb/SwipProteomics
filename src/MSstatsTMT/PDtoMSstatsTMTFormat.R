...
PDtoMSstatsTMTFormat <- function(input,
  ...
  input <- input[, which(colnames(input) %in% c(
    which.pro, which.NumProteins,
  ...
  if (nrow(fea.multimeas) > 0) { ## if there is any feature issued.
    ...
    input.new <- input.new[, -which(colnames(input.new) %in%
      c(
        "Quan.Info", "numProtein", "Ions.Score", "fea", "fea2",
    ...
    input.new <- input[, -which(colnames(input) %in% c(
      "Quan.Info", "numProtein", "Ions.Score", "fea", "fea2",
      "issue", "Isolation.Interference...."
    ))]
  }
  ...
  if (nrow(noruninfo) > 0) {
    for (i in seq_len(nrow(noruninfo))) {
      if (!quiet) {
        message(paste0(
          "** Annotation for Run : ", noruninfo[i, "Run"],
...
.combine.fractions <- function(data, quiet = TRUE) {
  ...
  for (i in seq_along(techruns)) { # for each technical replicate
    ...
    if (nrow(remove_peptide_ion) > 0) {
      ...
      if (!quiet) {
        message(
          "** For peptides overlapped between fractions of ", techruns[i],
...
.check.annotation <- function(annotation) {
  ...
  if (!all(required.annotation %in% colnames(annotation))) {
    ...
    stop(paste(
      "Please check the required column in the annotation file. ** columns :",
