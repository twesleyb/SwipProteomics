...
fit_full_model <- function(data) {
  message("fit: A ~ 1 + (1|M) + (1|M:R) + C + (1|B:C:M)")
...
fit_reduced_model_techrep <- function(data) {
  message("fit: A ~ 1 + (1|Run) + C + (1|B:C)")
  fit <- suppressMessages(try(lmerTest::lmer(Abundance ~ 1 + (1 | Run) + # whole plot
    Condition + # subplot
    (1 | BioReplicate:Condition), data = data), TRUE))
if (!inherits(fit, "try-error")) { return(fit) } else { # if the parameters are not estimable, return null
    return(NULL)
  }
}
...
.issue.checking <- function(data,
  ...
  sub_groups <- as.character(unique(data$Condition))
