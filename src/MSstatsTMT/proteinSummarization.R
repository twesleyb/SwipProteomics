...
#' After all, global median normalization on peptide level data and
#' normalization between MS runs using reference channels will be implemented.
#'
#' @export
#'
#' @importFrom utils read.table sessionInfo write.table
#'
#' @param data Name of the output of PDtoMSstatsTMTFormat function or
#' peptide-level quantified data from other tools. It should have columns
...
#'
#' @param global_norm  Global median normalization on peptide level data
#' (equalizing the medians across all the channels and MS runs). Default is
#' TRUE. It will be performed before protein-level summarization.
#'
#' @param reference_norm Reference channel based normalization between MS runs
#' on protein level data. TRUE(default) needs at least one reference channel in
#' each MS run, annotated by 'Norm' in Condtion column. It will be performed
#' after protein-level summarization. FALSE will not perform this normalization
#' step. If data only has one run, then reference_norm=FALSE.
#'
...
#'
#' @param remove_norm_channel TRUE(default) removes 'Norm' channels from protein
#' level data.
#'
...
proteinSummarization <- function(data,
  ...
  if (!all(required.info %in% colnames(data))) {
    ...
    stop(paste("Please check the required input. ** columns :",
      required.info[missedAnnotation], ", are missed.",
      collapse = ", "
    ))
  }
  ...
  norm.protein.data <- .protein.summarization.function(
    data,
    method,
    global_norm,
    reference_norm,
