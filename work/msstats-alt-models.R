...
fm6 <- lmerTest::lmer(fx6, data = msstats_prot %>% subset(Protein == prot))
...
message("fitting: ",
	paste(as.character(fx6)[2], as.character(fx6)[1], as.character(fx6)[3]))
...
for (prot in proteins) {
  fm <- lmerTest::lmer(fx6, msstats_prot %>% subset(Protein == prot), control = lmer_control)
