...
# calc rel intensity
swip_tmt <- swip_tmt %>% group_by(Protein) %>% dplyr::mutate(rel_Intensity = Intensity/sum(Intensity))
...
# Rel intensity: mixed-model with Subject nested within mixture
#fx7 <- log2(rel_Intensity) ~ 0 + Condition + (1|Mixture:Subject)
#fm7 <- lmerTest::lmer(fx6, data = swip_tmt %>% subset(Protein == prot))
#LT <- getContrast(fm7, "Mutant","Control")
#lmerTestContrast(fm7, LT) %>% dplyr::mutate(Contrast='Mutant-Control') %>% unique() %>% knitr::kable()
